...
USE MODD_PARAM_ICE_n, ONLY: PARAM_ICE_GOTO_MODEL, PARAM_ICEN_INIT, PARAM_ICEN, &
                          & CSUBG_AUCV_RC, CSUBG_AUCV_RI, LICE_T
...
USE MODD_TIWMX,          ONLY: TIWMX
USE MODD_ICET_PARAM,       ONLY: ICET_PARAM_GOTO_MODEL, ICE_T_PARAMETERS
...
IF(CMICRO=='ICE3' .OR. CMICRO=='ICE4' .OR. CMICRO=='LIMA') THEN
  ...
  IF(LLCHANGEMODEL) THEN
    ...
    CALL ICET_PARAM_GOTO_MODEL(KTO)
  ...
  IF(PRESENT(PHYEX_IN)) THEN
    ...
    ENDIF
    ICE_T_PARAMETERS = PHYEX_IN%ICE_T_PARAM
  ENDIF
  ...
  IF(LLINIT) THEN
    CALL INI_RAIN_ICE(HPROGRAM, KLUOUT, PTSTEP, PDZMIN, CLOUDPARN%NSPLITR, CMICRO, ICE_T_PARAMETERS)
    CALL INI_TIWMX
  ...
  IF(PRESENT(PHYEX_OUT)) THEN
    ...
    ENDIF
    PHYEX_OUT%ICE_T_PARAM = ICE_T_PARAMETERS
  ENDIF
ENDIF
!
...
IF(LLINIT) THEN
  ...
  TNSV%NSV=0
  CALL LIMA_UPDATE_NSV(PARAM_LIMA, TNSV, ODINIT=.TRUE., KMI=KTO, KSV=TNSV%NSV, &
                       &HDCLOUD=CMICRO, ODUPDATE=.TRUE.)
...
!**       CLOUD SCHEME
!
! Must be before microphysics (consistency checks in PARAM_LIMA_INIT)
...
IF(LLCHECK) THEN
  IF((CMICRO=='ICE3' .OR. CMICRO=='ICE4' .OR. CMICRO=='LIMA') .AND. CTURB=='TKEL') THEN
    ...
    ENDIF
    IF ((CSUBG_AUCV_RC == 'ADJU' .OR. CSUBG_AUCV_RI == 'ADJU') .AND. CMICRO=='LIMA') THEN
      CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'INI_PHYEX', &
                    &"CSUBG_AUCV_RC and/or CSUBG_AUCV_RI cannot be 'ADJU' if CMICRO is 'LIMA'")
    ENDIF
  ...
  IF(CMICRO=='LIMA') THEN
   IF (LSUBG_COND .AND. (.NOT. LPTSPLIT)) THEN
    ...
    END IF
    IF (CSCONV=='EDKF') THEN
      IF (PARAM_MFSHALLN%LRELAX_ALPHA_MF) THEN !PARAM_MFSHALLN%LRELAX_ALPHA_MF undefined if CSCONV/=EDKF
        CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'INI_PHYEX', & 
