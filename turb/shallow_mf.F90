...
DO JK=1, IKT
  ...
  END DO
END DO
CALL COMPUTE_FRAC_ICE(CST, NEBN%CFRAC_ICE_SHALLOW_MF,NEBN,ZFRAC_ICE(IIJB:IIJE,1:IKT),ZWK(IIJB:IIJE,1:IKT), IERR(IIJB:IIJE,1:IKT))
...
IF(PRESENT(BUCONF)) THEN
 IF( BUCONF%LBUDGET_U ) THEN
   ...
   CALL TBUDGETS(NBUDGET_U)%PTR%ADD_PHY(D, 'MAFL', ZWORK2)
 END IF
!
 IF( BUCONF%LBUDGET_V ) THEN
   ...
   CALL TBUDGETS(NBUDGET_V)%PTR%ADD_PHY(D, 'MAFL', ZWORK2)
 END IF
! 
 IF( BUCONF%LBUDGET_TH ) THEN 
   DO JK=1, IKT
     ...
     END DO
   END DO
   CALL TBUDGETS(NBUDGET_TH)%PTR%ADD_PHY(D, 'MAFL', ZWORK)
 END IF
!
 IF( BUCONF%LBUDGET_RV ) THEN
   DO JK=1, IKT
     ...
     END DO
   END DO
   CALL TBUDGETS(NBUDGET_RV)%PTR%ADD_PHY(D, 'MAFL', ZWORK)
 END IF
!
 IF( BUCONF%LBUDGET_SV ) THEN
   DO JSV=1,KSV
    ...
    ELSE
      ...
      END DO
    END IF
    CALL TBUDGETS(NBUDGET_SV1 - 1 + JSV)%PTR%ADD_PHY(D, 'MAFL', ZWORK )
   END DO
 END IF
END IF
!
