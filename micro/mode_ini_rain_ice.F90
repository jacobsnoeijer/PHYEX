...
!
!
!          1.0.1 Constants for possible modifying some processes related to 
...
!
!          1.0.2 Processes can be written two different ways: the OLD one only compatible
!                with the LSNOW_T=F, and the NEW one compatible with both LSNOW_T values.
!                Code rain_ice_old (LREF=F) in Meso-NH is written using the NEW way, except sedimentation
!                Code rain_ice_old (LRED=F) for other models is written using the OLD way
!                Code rain_ice (LRED=T) contains both formulation but the one used must be
!                in accordance with the computation of some coefficients done here.
!                The LNEWCOEFF controls this behaviour.
IF(.NOT. LRED) THEN
...
!
!
IF((.NOT. LNEWCOEFF) .OR. (HPROGRAM=='MESONH' .AND. .NOT. LRED)) THEN
